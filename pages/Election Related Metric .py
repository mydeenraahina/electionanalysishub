...
with col3: 
    def display_time():
        ...
        st.markdown(
            f'<p style="font-size: 16px; font-weight: italic; color: tomato;">⏰ {current_time}</p>',
...
path = "C:\\Users\\user\\AppData\\Local\\Programs\\Python\\Python311\\Detailed Results.xlsx"

pd.options.display.max_rows = 4070
pd.options.display.max_columns = 8

dataset4 =pd.read_excel(path)
...
class Clean_Dataset4:
    ...
    def droping_cols(self, dataset):
        ...
        drop_columns = ["STATE/UT NAME","AC NO.","CANDIDATE NAME","SEX","AGE","PARTY","CATEGORY","SYMBOL"]
        # Drop the specified columns
        dataset.drop(columns=drop_columns, inplace=True)
    def rename_cols(self, dataset):
        # Rename the column "ABBREVIATION" to "POLITICAL_PARTIES"
        dataset.rename(columns={"AC NAME": "DISTRICT"}, inplace=True)

    def setting_index(self, dataset):
        # Set 'PARTIES' column as the index
        dataset.set_index('DISTRICT', inplace=True)
    def removing_empty_val(self, dataset):
        # Removing empty values
        dataset.dropna(inplace=True)
